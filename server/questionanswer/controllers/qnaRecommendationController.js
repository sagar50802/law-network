...
class QnARecommendationController {
  ...
  async getRecommendations(req, res) {
    try {
      ...
      if (completedSubtopicId) {
        const dependentTopics = await this.getDependentTopics(completedSubtopicId);
      ...
      if (currentTopicId) {
        const nextTopics = await this.getNextTopics(currentTopicId);
      ...
      const difficultyTopics = await this.getSimilarDifficultyTopics(currentTopicId);
      ...
      recommendations = await this.enrichRecommendations(recommendations, userId);
  ...
  async getDependentTopics(completedSubtopicId) {
    try {
      // Find the subtopic
  ...
  async getNextTopics(currentTopicId) {
    try {
      // Get current topic
  ...
  async getSimilarDifficultyTopics(currentTopicId) {
    try {
      ...
      for (let rec of recommendations) {
        ...
        if (progress && rec.type === 'topic') {
          const topicQuestions = await Question.find({
            subtopicId: { $in: await this.getSubtopicIds(rec.id) },
            isReleased: true
          }).select('_id');
...
module.exports = new QnARecommendationController();
