...
const findPlaylist = (db, key) => db.playlists.find(p => (p._id || p.id || p.name) === key);
...
const upload = multer({ storage, limits: { fileSize: 800 * 1024 * 1024 } });
...
async function addItem(req, res) {
  ...
  const it = { id: uid(), title, url, createdAt: new Date().toISOString() };
  pl.items ||= [];
  pl.items.push(it);
  await writeDB(db);
