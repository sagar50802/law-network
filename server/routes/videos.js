...
const UP_DIR = path.join(process.cwd(), "server", "uploads", "videos");
...
let playlists = []; // [{ _id, name, items:[{ id, title, url, locked }] }]
...
router.post("/playlists/:pid/items", upload.single("file"), (req, res) => {
  const pid = req.params.pid;
  const pl = findPl(pid);
  ...
  if (!url && req.file) url = `/uploads/videos/${req.file.filename}`;
  if (!url) return res.status(400).json({ success: false, message: "No file or URL provided" });

  pl.items.push({ id, title, url, locked });
