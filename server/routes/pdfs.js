...
// CORS helper used on every response from this router
...
// ── JSON metadata (subjects/chapters) ─────────────────────
...
// Optional helpful logs (does not change logic)
...
// Wrap multer so errors become JSON (not 502) and keep CORS headers
...
// Accept either field name from the client: "pdf" or "file"
...
router.post("/subjects/:sid/chapters", uploadChapter, async (req, res) => {
  try {
    ...
    // Viewer URL (streams from GridFS)
  ...
  if (removed?.url?.includes("/api/gridfs/pdf/")) {
    ...
    try {
      if (mongoose.connection.readyState !== 1) {
        console.warn("GridFS delete skipped: Mongo not connected");
      } else {
...
router.delete("/subjects/:sid", async (req, res) => {
  ...
  try {
    if (mongoose.connection.readyState !== 1) {
      console.warn("GridFS subject cleanup skipped: Mongo not connected");
    } else {
      const bucket = new mongoose.mongo.GridFSBucket(mongoose.connection.db, {
        bucketName: "pdfs",
      });
...
// Route-level error handler (keeps CORS)
