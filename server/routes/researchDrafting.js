...
function clamp(n, min, max) {
  const x = Number(n || 0);
  return Math.max(min, Math.min(max, isFinite(x) ? x : 0));
}
...
function buildAbstract(title, subject, nature, baseAbstract) {
  ...
  const seed = [
    ...
    `This study, tentatively titled “${
      t || "A Study on " + sub
    }”, examines the contours, debates, and practical implications associated with the topic.`,
    nat === "empirical"
...
function buildReview(title, subject) {
  ...
  const para =
    `The review of literature around “${s(title) || subject}” clusters into recurring strands. ` +
    pts.join(" ") +
    ` Together, these strands chart the intellectual terrain and justify the present inquiry.`;
  return para;
}

function autoNature(nature, userNature) {
  if (userNature && ["empirical", "doctrinal"].includes(userNature))
    return userNature;
...
router.post("/admin/config", json, async (req, res) => {
  try {
    ...
    if (req.body.defaultAmount != null)
      c.defaultAmount = clamp(req.body.defaultAmount, 1, 99999);
...
/* ---------------------------- export route ---------------------------- */
/**
 * /api/research-drafting/:id/export?fmt=pdf
 * matches your frontend href
 */
router.get("/:id/export", async (req, res) => {
  try {
    ...
    // we only support PDF here, since docx is not installed
    if (fmt === "pdf") {
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader(
        "Content-Disposition",
      ...
      const pdf = new PDFDocument();
      pdf.pipe(res);
      pdf.fontSize(14).text(doc.title || "Research Drafting");
      pdf.moveDown();
