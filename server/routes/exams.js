...
const daysBetweenUTC = (a, b) =>
  Math.floor((floorUTC(a) - floorUTC(b)) / 86400000);
...
router.post("/", isOwner, express.json(), async (req, res) => {
  ...
  if (!examId || !name)
    return res
      .status(400)
      .json({ success: false, message: "examId & name required" });
  const doc = await Exam.findOneAndUpdate(
    { examId },
    { name, scheduleMode: "cohort" },
...
router.get("/:examId/templates", async (req, res) => {
  const list = await ModuleTemplate.find({ examId: req.params.examId })
    .sort({ dayIndex: 1, slot: 1 })
    .lean();
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!di || !title || !releaseAt) {
      return res
        .status(400)
        .json({ success: false, message: "dayIndex, title, releaseAt required" });
    }
    ...
    const pushFile = async (buf, orig, mime, type) => {
      const { url } = await uploadBuffer({
        keyPrefix: `exams/${req.params.examId}`,
        originalName: orig,
        buffer: buf,
        contentType: mime,
      });
    ...
    const addFrom = async (field, type) => {
      ...
      for (const f of arr)
        await pushFile(f.buffer, f.originalname, f.mimetype, type);
    };
...
router.patch("/:examId/templates/:tid", isOwner, upload.none(), async (req, res) => {
  ...
  ["title", "description", "slot", "releaseAt"].forEach((k) => {
    if (req.body[k] != null)
      patch[k] = k === "releaseAt" ? new Date(req.body[k]) : req.body[k];
  });
  if (req.body.dayIndex != null) patch.dayIndex = Number(req.body.dayIndex);
  if (req.body.flags != null)
    patch.flags =
      typeof req.body.flags === "string"
...
router.delete("/:examId/templates/:tid", isOwner, async (req, res) => {
  const doc = await ModuleTemplate.findOneAndDelete({
    _id: req.params.tid,
    examId: req.params.examId,
  });
  if (!doc)
...
router.post(
  "/:examId/templates/quick-schedule",
  isOwner,
  express.json(),
  async (req, res) => {
    const {
    ...
    for (let i = 0; i < Number(days || 30); i++) {
      ...
      items.push({
        ...
        flags: {
          ...
          showOriginal:
            defaults.showOriginal != null ? !!defaults.showOriginal : true,
...
router.get("/:examId/access", async (req, res) => {
  ...
  if (email) {
    const acc = await ExamAccess.findOne({
  ...
  // admin list
  const items = await ExamAccess.find({ examId: req.params.examId })
    .sort({ createdAt: -1 })
    .lean();
  res.json({ success: true, items });
});
...
router.post("/:examId/access/grant", isOwner, express.json(), async (req, res) => {
  ...
  await ExamAccess.updateMany(
    { userEmail: email, examId: req.params.examId, status: "active" },
    { $set: { status: "archived" } }
  );
  const created = await ExamAccess.create({
    userEmail: email,
    examId: req.params.examId,
...
router.get("/:examId/overview", async (req, res) => {
  ...
  const acc = email
    ? await ExamAccess.findOne({ userEmail: email, examId, status: "active" })
    : null;
  ...
  res.json({
    ...
    completed,
    access: acc
      ? { startAt: acc.startAt, expiryAt: acc.expiryAt, planDays: acc.planDays }
      : null,
  });
});
...
router.get("/:examId/today", async (req, res) => {
  ...
  const modules = await ModuleTemplate.find({ examId, dayIndex })
    .sort({ slot: 1 })
    .lean();
...
router.post("/:examId/complete-day", express.json(), async (req, res) => {
  ...
  if (!email || !dayIndex)
    return res
      .status(400)
      .json({ success: false, message: "email & dayIndex required" });
...
/* ✅ NEW: overlay/meta endpoints (compatible with Day & Time config) -------- */
...
// GET overlay-config (admin) — returns Day & Time shape
...
// Save overlay-config (admin) — accepts JSON body (POST or PATCH)
function normalizeOverlayBody(b = {}) {
  const modeList = ["planDayTime", "afterN", "fixed", "never"];
  const mode = modeList.includes(b.mode) ? b.mode : "planDayTime";
  const showOnDay = Number(b.showOnDay) || 1;
  const showAtLocal = String(b.showAtLocal || "09:00").slice(0, 5);
  const daysAfterStart = Number(b.daysAfterStart) || 0;
  const fixedAt = b.fixedAt || null;
  return { mode, showOnDay, showAtLocal, daysAfterStart, fixedAt };
}
...
async function saveOverlayHandler(req, res) {
  ...
  const overlay = normalizeOverlayBody(b);
  const set = {
    overlay,
    ...(b.price != null ? { price: Number(b.price) || 0 } : {}),
    ...(b.trialDays != null ? { trialDays: Number(b.trialDays) || 0 } : {}),
  };
  ...
  return res.json({ success: true, exam: doc });
}
